function [pts_master, pts_aligned,dist] = RegCorr( master,aligned,options)%align,corr_val,scale )
...
for i=1:master_points1.size%pts_array_size
    if(manual==1)
        ...
        current_loc= master_points1(i);%pts_array_x(i);
        x_cood=current_loc.Location(1);%pts_array_y(i);
    ...
    if(((x_cood-d1) >= 0) &&  ((y_cood-d1) >= 0) && ((x_cood+d1) < 1280) && ((y_cood+d1) < 960))
        ...
        if(((x_cood-d2) >= 0) &&  ((y_cood-d2) >= 0) && ((x_cood+d2) < 1280) && ((y_cood+d2) < 960))
            ...
            %% To search master template in searcg template image
            if(mean2(ref_template)==ref_template(1,1))
                continue;
            end
            normed=xcorr2(double(search_template),double(ref_template));
            if(max(normed(:))>options.corrThreshold)
                ...
                if (dist_sq<options.distance&& align==1 && x_cood>=0&& aligned_x_cood>=0&& y_cood>=0&& aligned_y_cood>=0)% Select pts only with 1 pxl diff
